...
const search = ref<{supplier:string; order_no:string; range: [Date,Date]|null}>({ supplier:'', order_no:'', range: null })
...
const savePaid = async () => {
  try {
    ...
    await execute('INSERT INTO payables (order_no, supplier, ship_date, total_amount, paid_amount, unpaid_amount) VALUES (?,?,?,?,?,?) ON CONFLICT(order_no) DO UPDATE SET paid_amount=?, unpaid_amount=?, updated_at=CURRENT_TIMESTAMP', [editOrderNo.value, '', '', editTotal.value, paid, unpaid, paid, unpaid])
